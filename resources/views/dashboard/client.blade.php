...
<script>
                ...
                data.forEach(job => {
                    let html = `
                        <tr>
                            ...
                            <td><button class="btn btn-success btn-sm apply-job-btn" data-job-id="${job.job_id}">Apply</button> </td>
                        </tr>
                    `
    ...
    document.addEventListener('click',(event)=>{
        if(event.target.classList.contains('apply-job-btn')){
        let job_id = event.target.getAttribute('data-job-id');
        formData = new FormData();
        formData.append('job_id',job_id)
        formData.append('_token','{{csrf_token()}}')
        var req = new Request('{{url("/job/apply")}}', {
            method: 'post',
            body : formData
        });
        fetch(req)
